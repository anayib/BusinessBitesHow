class CoursesController < ApplicationController
	...
	private
		def course_params
  		params.require(:course).permit(:title, :description, :new_course, :image, :course_type, { :instructor_ids => [] }, { :category_ids => [] }, lessons_attributes: [:id,:title,:description,:_destroy, topics_attributes: [:id,:title,:description,:content_url, :activities, :document, :transcription, :_destroy, resources_attributes: [ :id, :title, :description, :link_url, :_destroy]]])
		end
